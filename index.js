...
function superbowlWin(element) {
    for (const result of element) {
        if (record.find=== "W") {
           return `${year} the year the Denver Broncos won the Super Bowl`
          //console.log(record.year);
        }
    }
}
superbowlWin(result, "W")*/
const isWin = function (element) {
    return element.result === "W";
};
function superbowlWin(record) {
    const wins = record.find(isWin);
    //(record => record.result === "W");
    // if (wins === true) {
    //     return wins.year} {
    // else if (wins !== true) {return undefined}
